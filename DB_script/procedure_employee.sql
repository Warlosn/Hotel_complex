...
insert into Busy_room(room_id, user_id_,date_checkIn,date_checkOut,price) values
...
update Rooms set room_isFree = 1  from Rooms inner join Busy_room on Rooms.room_id=Busy_room.room_id where Busy_room.busy_id=@idBusyRoom;
...
select @serviceTypePrice = services_type_price from Services_type where services_type_id = @serviceTypeId;
update Busy_room set price = @lastprice + @serviceTypePrice  where busy_id=@idBusy;
end;
go
----------------------------------------------Просмотр заказанных сервисов------------------------------------------------------------
...
as
begin
select Services_.busy_id, Services_type.services_type_name,Services_type.services_type_price from Services_ inner join Services_type on Services_.service_type=Services_type.services_type_id where busy_id=@busyId;
end;
go