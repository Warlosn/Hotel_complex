...
select @check = type_id_ from Room_type 
where type_name_ =@type;
select @countR = count(*) from Rooms 
where Rooms.room_isFree=1 and room_idType=@check;
return @countR;
end;
go
...
insert into BookedRooms(room_id, user_id_,date_checkIn,date_checkOut,price) 
